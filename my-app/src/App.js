...
function App() {
  ...
  const fetchLatestRecords = async () => {
    try {
      console.log(process.env.DISEASE_APP_PORT);
      const response = await axios.get(`http://localhost:5354/latest_records`);
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      const infoResponse = await axios.post(`http://localhost:5354/patient_counts_info`, {
      ...
      const diseasePromises = diseaseNameList.map(async (diseaseName, index) => {
        const diseaseResponse = await axios.post(`http://localhost:5354/patient_counts_disease`, {
          info_id: newInfoId,
          sr_no: index + 1,
      ...
      const updatedDiseaseRecords = await Promise.all(
        diseaseNameList.map(async (diseaseName) => {
          try {
            ...
            // Find the corresponding record from newDiseaseRecords
            const recordToUpdate = newDiseaseRecords.find(
              record => record.disease === diseaseName && record.info_id === newInfoId
            );
  
            // Update the database with the response data
            if (recordToUpdate) {
              
              const updateResponse = await axios.put(`http://localhost:5354/patient_counts_disease/${recordToUpdate.id}`, {
                visits: diseaseCount.visits,
                patients: diseaseCount.patients,
                discharge_summaries: diseaseCount.discharge_summaries,
                ds_patients: diseaseCount.ds_patients,
                status: 'Completed'
              });
  
              // Merge the updated record with the original record
              const updatedRecord = { ...recordToUpdate, ...updateResponse.data };
              return updatedRecord;
            } else {
              console.error(`No record found for disease: ${diseaseName} with info_id: ${newInfoId}`);
            }
          } catch (error) {
      ...
      const filteredUpdatedRecords = updatedDiseaseRecords.filter(record => record !== undefined);
  ...
  const recordsTable = latestRecords.length > 0 ? (
